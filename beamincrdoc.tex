...
\end{verbatim}
  ...
  \item[\tt \string$*] \hypertarget{incrref$*}{}The highest numbered overlay in
    the frame used so far.  This may be useful to continue the display of a
    partially rendered frame, starting from the latest point reached in any
    previous display.
...
than the current increment and to the largest of them if none are larger.  This
label can be used to easily synchronise later incremental environments to a
first one, even if each \ttc\next\ step involves multiple increments.  \example
\begin{verbatim}
  \begin{<>}
...
If \TikZ\ is loaded, \beamincr\ defines various graphical alerts or
\ttx{glerts}.  The argument to be alerted is set within an inline \TikZ\ node
aligned with the baseline of the surrounding line, with a tight bounding box.  A
second node or associated path is used to add the graphical embellishment. (More
details of the construction are given in \cref{sec:attention:glerts:refining}
below.)  Glerts can be used to draw attention at specific increments, or simply
to highlight material on the slide in a static fashion.
...
style as surrounding text when inactive.  Their actions when activated are as
follows.
\begin{description}
...
\item[\tt connect=\{\oarg{to options} (\meta{target})\}] Draw a (red, very
  thick, double-headed arrow) line to the glert (or other \TikZ\ node) named
  \meta{target}.  The picture containing the node must have the \ttx{remember
    picture} option; this is always the case for glerts.  The \ometa{to options}
  can modify the shape of the path.  The style can be changed with
  \ttx{path=\marg{options}}.
...
\item[\tt callout=\{\oarg{callout node options}\meta{angle}:\meta{callout text}]
  ...
  from \meta{angle} (may be a direction keyword).  The default callout is
  rectangular and placed at a centre-to-centre distance 2.5 times greater than
  that from the glert node centre to its edge.  The \ometa{callout node options}
  can change its appearance, and placement (in particular, the \ttx{pos} option
  modifies the scaling relative to the glert node size).
...
These can also be set as separate options as described in
...
The name of the glert node can be set with the option
\ttx{node=\{name=\meta{name}\}}, or, more simply, \ttx{(\meta{name})}.  The node
can then be referenced by a \ttx{connect} glert, or from another \TikZ\ picture
(remembering to use \ttx{remember picture} and \ttx{overlay} as appropriate).
...
the argument as there would be with other boxed commands.  However,
\TeX\ internals mean that to achieve this transparency, both the maths material
and associated glert drawing must be rendered interally four times (for each of
the four maths styles), with only one rendering being placed on the output page.

Although this requirement can increase compile time, the consequences are
generally acceptable.  However, it does create a difficulty when using named
glerts (say, with the \ttx{connect} glert style).  The same name is associated
with all four nodes in turn, finally being assigned to the
\ttc\scriptscriptstyle\ version.  If that isn't the one that is used in the
final layout, then the name ends up being associated with the bottom corner of
the page.

Thus, \beamincr\ makes it possible to specify the correct mathstyle explicitly,
shortcircuiting the need for the four versions.  This is done by placing the
style instruction as the \emph{first} token in the \ometa{options} to the
\cmd\glert\ttx{[*]} command.  If a \meta{glert style} or additional options are
to be provided, these must follow.  They can be separated from the mathstyle
command by a comma, but this is not necessary.

\example
\begin{verbatim}
\begin{equation*}% simple, but naming issues, and possibly inefficient
\int_{\glert[(lo)]{-\infty}}^{\glert[(hi)]{+\infty}} e^{-i\omega t} d\omega = 2\pi\delta(t)
\end{equation*}

\begin{equation*}% contents rendered once in the specified style
\int_{\glert[\scriptstyle (lo)]{-\infty}}^{\glert[\scriptstyle (hi)]{+\infty}}
       e^{-i\omega t} d\omega = 2\pi\delta(t)
\end{equation*}
\end{verbatim}

Note that math node namingdoesn't seem to work in \ttx{align} environments.
...
declaration:
%
\begin{commandalt}[<object> <state> (tikz option)]{{option:\meta{object} \ometa{state}}=\marg{options}}
...
%
  ...
  This option must be placed within the argument to a \cmd\useglert\ or the
  options to \cmd\glert\ or \cmd{\glert*}.  It appends the \meta{options} to the
  \ttx{glert \meta{object} \meta{state}} style, thus modifying its appearance.
  If \meta{state} is omitted it is taken to be \ttx{active}.
  ...
  and explicit style.  Options within \cmd\glert\ttx{[*]}\ are local to that
  instance.
...
\begin{commandalt}[glert <glert style> <object> <state> (tikz option)]
  {{option:glert \meta{glert style} \meta{object} \meta{state}/.append
  ...
  \meta{glert style} glerts.  This option can be set within \ttc\tikzset,
  \cmd\useglert, or \cmd\glert\ttx{[*]}.  In the first two cases it applies
    throughout the current \TeX\ block.  In the latter, it applies just to the
    glert in question (and so is effectively just a more verbose version of the
    option above---with the exception of subtle effects possible when applying
    multiple \meta{glert style}s.)
    \example
\begin{verbatim}
...
picture: however the \ttx{bbox} node sets its own default explicitly and so will
be unaffected.  If you want to increase the space reserved for the text, use
\example
\begin{verbatim}
\glert{box, bbox={inner sep=4pt}}{text}
\end{verbatim}
instead.
...
\begin{command}{\defineglertpath\marg{glert style}\marg{state}\marg{tikz path commands}}
  ...
  %
  Add the following option to the \meta{glert style} node \meta{state} style:\\[1ex]\ttx{
...
Here are the definitions of some of the builtin glert styles:
\begin{verbatim}
\makeglertstyle{alert}
  \defineglertstyle{alert}{node}{active}[red]{text=#1}
\makeglertstyle{box}
  \defineglertstyle{box}{node}{active}[red]{draw=#1,very thick}
\makeglertstyle{ellipse}
  \defineglertstyle{ellipse}{node}{active}[red]{ellipse,draw=#1}
\makeglertstyle{highlight}
  \defineglertstyle{highlight}{node}{active}[red]{fill=#1,fill opacity=0.2,text opacity=1}
...
\makeglertstyle{spotlight}
  \defineglertstyle{spotlight}{node}{active}{ellipse,fill=red,path fading=spotlight,#1}
\makeglertstyle{connect}
  \defineglertpath{connect}{active}{(\glertnode) to #1}
  \defineglertstyle{connect}{path}{active}{overlay,draw,red,very thick,<->}
\end{verbatim}
