...
\def\bkmkenv#1#2{%
  \belowpdfbookmark{\textbackslash begin\{#1\}\dots\textbackslash end\{#1\}}{#1}%
  \addcmdtoloc{#1}{\textbackslash begin\{#1\}\dots\textbackslash end\{#1\}}{}%
}
...
\begin{command}{\resetincr\oarg{incrnum}}%
  ...
  %
  This command resets the increment number to 1, or to the value defined by the
  optional argument if given.  It doesn't directly affect the slide on which
  subsequent text appears, but it does change effect of subsequent \texttt{<+>}
  or \texttt{<.>} increments.  The command may be useful to synchronise overlays
  in (say) two columns or between highlighted bullet points and highlighting in
  a figure.  \example
\begin{verbatim}
\begin{frame}
  \resetincr
...
%
  ...
  special label `\ttx!'  is introduced in \cref{sec:actions:!}.  The
  ...
  specification by enclosing them with slashes, e.g. \ttx{</foo(2)/>}.
\end{commandalt}
\example
\begin{verbatim}
...
specifications that include increment specs.
...
There is also a similar new command called \cmd\handoutframe\ to render
more than one overlay from a frame in \ttx{handout} or similar modes that, by
default, just show a single slide with all the overlays collapsed.  See
\cref{sec:again}.
...
\ttx{block} and \ttx{theorem}.  This package adds the fields of
...
\ttc\action\ acts like \ttc\uncover\ (or \ttc\onslide\ with an
argument).
...
\end{commandalt}
\vskip-3ex
\begin{commandalt}[<!\meta{incrnum}>][<!incrnum>]{{<!}\meta{incrnum}\opt{\tt-\meta{incrnum}}\contcmd{>}}{}
It is possible to use the \beamer\ syntax for actions in overlays to reset the
increment number.  This can be done using the explicit
\ttx{resetincr@\meta{incrnum}} action or with an implicit
\ttx{<!\meta{incrnum}>} specification.  \example
\begin{verbatim}
...
\begin{verbatim}
  ...
  \onslide<+->{this on 5+}
\end{verbatim}
The forms \ttx{<!+>} and \ttx{<!.>} aren't supported (and wouldn't be
useful: \ttx{<+>} already advances the increment, while \ttx{<!.>} would
set it to its current value).  However \ttx{<!/.(\meta{offset})/>} (note the
\ttx/ label notation) can be used to advance the increment counter by
multiple (or negative) steps.

The reset takes effect after the overlay specification has been interpreted and
before the body is set.  So any \ttx+ or \ttx. references will be relative
to the increment in effect \emph{before} the \ttc\action.  However, the
special increment label \ttx{/!/} can be used to access the most recent
reset.  \example
\begin{verbatim}
  \resetincr
  ...
  \action<!/.(2)/-|alert@/!/>{alert when uncovered}
\end{verbatim}
It is possible to issue multiple \ttx! commands in one overlay spec but only
the first of them will take effect.

Actions must be used with argument text (usually enclosed in braces) or as
environments.  There is no equivalent to the declaration form of
\ttc\onslide.  Note, however, that
\cmd\fromslide\ (\cref{sec:setting}) implements an
\ttc\onslide\ declaration while also setting the current increment.
\end{commandalt}
...
\ttx{<\meta{incrnum}>} argument to \ttc\incrlabel.  The same effect can
...
default spec, but only assign a label on selected steps.  However, this
...
\ttx{//} in an \ttc\item, although the label can be empty (omission is
fine in the fields of an incremental or incremental alignment environment).
Second, if there happens to be one more or more \ttx{/} characters in the
argument to the action, the text between them (or from a single \ttx{/} to
the end) will be interpreted as a label, unless an explicit \ttx{//} pair
appears first.  You have been warned!
\end{commandalt}
...
overlay specification.  In this form, the \ttx\textasciitilde\ may be
proceeded by, sat, a mode specification, but must not have any following text
with the specification field (i.e. to the next \ttx\textpipe\ or \ttx>).
\example
\begin{verbatim}
  \begin{itemize}[<+-| alert@+|=>]
...
\begin{commandalt}[<defaultspec@>]{{<defaultspec@}\meta{range}\contcmd{>}}{}
  ...
  action argument.  The \meta{increment range} may be any valid specificiation
  for a set of slides (such as \ttx{1}, \ttx{+-} or
  \ttx{!/foo/-/bar/}), but may not itself contain any actions.  The range is
  evaluated within the context of the \ttx{defaultspec@} specification,
  yielding specific slide numbers.  Thus, any labels (including \ttx{/./} and
  \ttx{/!/}) or \ttx+ or \ttx. symbols will be replaced by their
...
\begin{commandalt}[<\textasciitilde range>][tilde range]{{<\textasciitilde}\meta{range}\contcmd{\textpipe$\cdots$>}}{}
  ...
  This is equivalent to \ttx{<\meta{increment range}\textpipe
    defaultspec@\meta{increment range}\textpipe$\cdots$>}.  That is, it executes
  the calling action on \meta{increment range} (using normal overlay evaluation
  rules) and also set the default specification within the action argument to
  the evaluated range.
  %
...
\begin{verbatim}
  ...
  \end{itemize}
\end{verbatim}
    The \ttc\only\ commands are used to advance the counter on slides
    where the \ttc\item s are visible.  In both cases the overlay
    specification to \ttc\only\ is a copy of the surrounding default
    specification.  For the first one, this is \ttx{<+->}.  For the second,
    it is \ttx{<+->} \emph{evaluated} within the \ttc\item\ call,
    giving \ttx{<3->}.  The final case expands the second |~| to the default
    \ttx{+-}, which becomes the increment specification for the item, and
    then sets the default within the item to its value, which is \ttx{4-}.
\end{commandalt}
...
increment specifications within alignment environments such as \ttx{tabular}
or \AmSTeX\ \ttx{align}; these are discussed in \cref{sec:align}.
...
\begin{envwithcontents}{{incremental}
    \opt{\ttx{[<}\meta{default specification}\ttx{>]}}}
...
%
  ...
  \opt{\meta{default specification}} is applied.  If no \meta{default
...
\end{verbatim}
\end{envwithcontents}
\begin{environment}{{<>}\opt{\ttx{[<}\meta{default specification}\ttx{>]}}}
...
\begin{envwithcontents}{{incrementaldo}
    \marg{command definition}\opt{\ttx{[<}\meta{default specification}\ttx{>]}}}
  {\item \hskip1em \osarg{pre-next specification} \opt{\meta{pre-next contents}}
    \item \ttx{\string\next}\osarg{next specification}
    \item \hskip1em \meta{next contents}
    \item \ttx{\string\next}\osarg{next specification}
...
\end{envwithcontents}
\begin{environment}{{do<>}\marg{command definition}\opt{\ttx{[<}\meta{default specification}\ttx{>]}}}
  This is a synonym for |\begin{incrementaldo} ... \end{incrementaldo}|.
\end{environment}


\begin{envwithcontents}{{incrementaldocmd}
    \oarg{num args}\marg{code}\opt{\ttx{[<}\meta{default specification}\ttx{>]}}}
  {\item \hskip1em \osarg{pre-next specification} \opt{\meta{pre-next contents}}
    \item \ttx{\string\next}\osarg{next specification}
    \item \hskip1em \meta{next contents}
    \item \ttx{\string\next}\osarg{next specification}
...
\begin{envwithcontents}{{incrementaldodef}
    \oarg{parameter spec}\marg{code}\opt{\ttx{[<}\meta{default specification}\ttx{>]}}}
  {\item \hskip1em \osarg{pre-next specification} \opt{\meta{pre-next contents}}
    \item \ttx{\string\next}\osarg{next specification}
    \item \hskip1em \meta{next contents}
    \item \ttx{\string\next}\osarg{next specification}
...
\begin{envwithcontents}{{incrementallayers}
    \oarg{node options}\opt{\ttx{[<}\meta{default specification}\ttx{>]}}}
  {\item \hskip1em \osarg{pre-next specification} \opt{\meta{pre-next contents}}
  ...
  |t|, |c| and |b| alignment respectively.  The nodes themselves are accessible
  ...
  layer, or a |tcb| alignment option to \ttx{boundinglayerscope}.  The origin
...
Standard \LaTeX\ alignment environments including |tabular| and the \ttx{align}
and \ttx{align*} environments from the \ttx{amsmath} package are not
...
\end{environment}
\begin{environment}{{align*<>}\opt{\ttx{[<}\meta{spec1}\ttx{>\&<}\meta{spec2}\ttx{>\& ...]}}}
  Each pre-processes the input to |align*|, placing an |\action<>{}| command
  around each field, defined as the material appearing between successive |&|,
  |\\| or end environment tokens.  By default, the first field on a line is called with
  ...
  fields on a single line, then the sequence is repeated.  The default
  ...
  increment resets, |=| label assignments or |~|\meta{range} default
  specification.
\example
\begin{verbatim}
...
\begin{environment}{{incrementaltabular}\oarg{pos}\marg{cols}\opt{\ttx{[<}\meta{spec1}\ttx{>\&<}\meta{spec2}\ttx{>\& ...]}}}
\end{environment}
\begin{environment}{{tabular<>}\oarg{pos}\marg{cols}\opt{\ttx{[<}\meta{spec1}\ttx{>\&<}\meta{spec2}\ttx{>\& ...]}}}
...
\begin{environment}{{incrementaltabular*}\oarg{pos}\marg{width}\marg{cols}\opt{\ttx{[<}\meta{spec1}\ttx{>\&<}\meta{spec2}\ttx{>\& ...]}}}
\end{environment}
\begin{environment}{{tabular*<>}\oarg{pos}\marg{width}\marg{cols}\opt{\ttx{[<}\meta{spec1}\ttx{>\&<}\meta{spec2}\ttx{>\& ...]}}}
...
\begin{command}{{\CreateIncrementalAlignmentEnvironment}\rlap{\marg{name}\oarg{Nopts}\marg{Nreqs}%
  ...
  |incremental|\meta{name} or \meta{name}|<>|.
\end{command}
...
\begin{commandalt}[<eqnum@>]{{<eqnum@}\meta{range}\contcmd{>}}{}
  Place the current equation number (as |\theequation|) on slides in
...
|\usenonincrementalenv{gather}| will make |gather| a synonym for |gather*|.  At
this point, there would be no easy way to restore |gather| to its original
...
\begin{commandalt}[<point@>]{{<point@}\meta{range}\contcmd>}{}
  ...
  above.  In \ttx{itemize} environments it replaces the default item label
  with the pointer\footnote{Although if the \ttx{itemize} is nested within an
    \ttx{enumerate}, it inherits the \ttx{enumerate} behaviour.}.  In
  \ttx{enumerate} environments it prepends the pointer to the default label.
...
\begin{commandalt}[<pointers@>]{{<pointers@}\meta{range}\contcmd>}{}
  This action activates |\point*| commands in the argument contents, but does
  not insert a pointer.
\end{commandalt}

\begin{command}{{\point*}\oarg{options}\opt{\marg{contents}}}
  {insert a pointer when activated (possibly using a \TikZ\ node around \ometa{contents})}
  If the command is not followed by a |[| or |{| character, insert a pointer when an
      enclosing |\point| command or action is active.

  If followed by an argument in |[]| or |{}|, and if \TikZ\ is loaded, call
  |\pointtonode| as described below.  If \TikZ\ is \emph{not} loaded a normal
...
If \TikZ\ is not loaded, this is the equivalent of |\point*|\marg{contents}, ignoring any
options given.
\end{command}
...
\ttc\beamer@defaultospec\ (the current default overlay specification) and
\ttc\beamer@againname\ (the label of the current frame, used to identify it in
calls to \cmd\againframe).  In particular, \ttc\beamer@againname\ is
incorporated into the internal name associated with \beamincr\ labels.
