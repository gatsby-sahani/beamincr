...
\begin{envwithcontents}{{incrementallayers}
  ...
  \begin{description}
  \item[\tt t\OR c\OR b] These options do not alter anything by themselves.
    ...
    recent layer node.
  \item[\tt layer xy] Set the \ttx{xy} coordinate system to span the bounding box of
    the layers rendered so far.  The origin depends on the alignment of the most
    recent layer, or a \ttx{t\OR c\OR b} alignment option to the \ttc{\next*}
    scope.  The origin is placed at the \ttx{north west}, \ttx{center} or
    \ttx{south west} of the bounding node, for \ttx t, \ttx c, or \ttx b
    alignment respectively.  The \ttx x vector extends horizontally to the east
    border of the bounding node, while \ttx y extends vertically to the opposite
    side (or top for \ttx c alignment).
  \begin{center}
  ...
  \item[\tt layer grid] Draw a grid within the current \ttx{layer xy} coordinates.
    Fine grid lines are 0.1 units apart.  Heavier lines are placed at full units.
  \end{description}
...
\item[\tt callout=\{\oarg{callout node options}\meta{angle}:\meta{callout text}\}]
  ...
  and placement.  In particular, the \ttx{pos=\meta{scale}} option modifies the
  position scale relative to the glert node ``radius'', and
  \ttx{sep=\meta{dimen}} adds a further offset.  If \meta{angle} is a a keyword,
  \ttx{pos} moves along the corresponding direction defined by the glert node
  shape, whereas \ttx{sep} moves in the corresponding absolute direction
  (e.g. $45^\circ$ for \ttx{north east}).
