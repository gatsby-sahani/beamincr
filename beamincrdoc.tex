...
\end{verbatim}
  ...
  Note that in modes such as \ttx{handout}, which ordinarily limit the number of
  slides generated per frame, this label and related ones will evaluate to 1.
  An alternative to access the final increment number (|$+|) is described below.
  ...
  \item[\tt \string$+] \hypertarget{incrref$+}{}The increment number in effect
    at the end of the frame.  This may differ from |$| or |$*| for many reasons:
    a higher overlay number may have been used explicitly; the increment number
    may have been reset after the highest-numbered increment was used; the mode
    may limit the number of slides generated; and more.
...
\begin{verbatim}
  \againframe<1,/halfway/,/$+(-1)/-/$+/>{twolists}
\end{verbatim}
provides an abbreviated tour of the lists, visiting the first, middle and last
two overlays.  Increment labels are associated with the label or number of the
enclosing frame, and so the same names can safely be reused across multiple
frames.

There is a similar new command called \cmd\handoutframe\ to render more than one
overlay from a frame in \ttx{handout} or similar modes that otherwise just show
a single slide with all the overlays collapsed.  See \cref{sec:again}.
...
discussion of \cmd\allowundefinedincrrefs\ for more on 0-valued references,
but note that this behaviour for \ttx{/!/} is independent of the
\cmd\allowundefinedincrrefs\ state.
%
\example
\begin{verbatim}
  \resetincr
...
\begin{verbatim}
  ...
  \action<!/.(3)/|=>{/(-1)x/action text}
\end{verbatim}
the label \ttx{x} is set to 5 by every one of the actions.  The last three cases
illustrate the use of assignment offsets: if \ttx{=} is followed by a number in
parentheses, this is treated as an offset to add to the current increment at
assignment, in the same way as indicated by the optional \ttx{<\meta{incrnum}>}
argument to \cmd\incrlabel.  The same effect can be achieved by placing the
offset before the label name within the enclosing \ttx{//}.
...
might apply.  It may sometimes be convenient to extend the default instead.  The
...
This extension is available in any overlay specification.
\end{commandalt}
...
\begin{command}{\allowundefinedincrrefs\oarg{flag}}%
...
An \cmd\allowundefinedincrrefs\ command also makes it possible to
...
\begin{envwithcontents}{{tikzpicture<>}
    \oarg{tikz options}\optsarg{default specification}}
...
\begin{tikzpicture}[options]
  \begin{incremental}[<default spec>]
    ...
  \end{incremental}
\end{tikzpicture}
\end{verbatim}
...
%
    The origin of the coordinates depends on the alignment of the most recent
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  ...
  \hfill\OR            & \tt \hyperlink{incrref$+}{|$+|\ooffset}  & = \tt \meta{increment number at end of frame}\poffset\\
  ...
  \hfill\OR          & \tt /\meta{incrref}-/ & = {\tt0-0} if \meta{incrref}
                                                      ...
                                                      |\allowundefinedincrrefs| is true\\ 
