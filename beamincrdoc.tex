...
\end{verbatim}
It's possible to issue multiple |!| commands in one overlay spec but only the
...
declaration while setting the increment.
...
\begin{environment}{{incralign[*]}\opt{\texttt{[<}\meta{spec1}\texttt{>\&<}\meta{spec2}\texttt{>\& ...]}}}
    ...
    |\action<+->{}|, and remaining ones with |\action<.->{}|.  This has the
    effect of displaying a full line at a time.  The optional argument makes it
    possible to change this to |\action<|\opt{\meta{spec1}}|>|,
    |\action<|\opt{\meta{spec2}}|>|, etc.\ with the sequence of specifications
    reset to |<|\opt{\meta{spec1}}|>| at the beginning of every line. If there are fewer
    ...
    |action@|\meta{increment} actions and implicit (|!| prefixed) increment
    resets.
...
\begin{command}{\framescanonlystate \marg{onoff}}
  This commands disables (when \meta{onoff} = 0) or enables (when \meta{onoff} >
  0) the effect of |\framescanonly|.  It is needed when a |\framescanonly| is
  used in |<beamer>| mode, and the frame is then accessed with |\handoutframe|
  or |\againframe|.  These commands will attempt to set the frame in |<beamer>|
  or the current mode, respectively, thus invoking the |\framescanonly| call
  within the frame again and suppressing output.  The solution is to temporarily
  disable its effect with |\framescanonlystate{0}|, restoring after the
  |\againframe| with |\framescanonlystate{1}|
\end{command}
