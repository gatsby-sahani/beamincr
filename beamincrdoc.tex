...
\item[\tt label=\{\oarg{path options}\meta{angle}:\oarg{label node
    options}\meta{label text}\}] Place a label near the glert node.  This is
  similar to the \TikZ\ label and pin options, but allows more automated
  placement and orientation control (beside, of course, being controlled by the
  \cmd\glert\ incremental system).

  The  \meta{path options} may include:
  \begin{description}\topsep=0pt
  \item[\tt pos=\meta{scale}] Scale separation from glert node in proportion
    to the glert node ``radius''.  This is similar to the \TikZ\ \ttx{pos}
    option for placing a node on a line, where the line in question extends from
    the centre of the glert node to its border.

  \item[\tt sep=\meta{dimen}] Add \meta{dimen} to the separation from glert
    node.  If \meta{angle} is a a keyword, \ttx{pos} moves along the
    corresponding direction defined by the glert node shape, whereas \ttx{sep}
    moves in the corresponding absolute direction (e.g. $45^\circ$ for
    \ttx{north east}).
    ...
    calculated by the \TikZ\ node coordinate system), while \ttx{corner} refers
    to \ttx{(\meta{glert node}.\meta{angle})}.  These may differ if \meta{angle}
    is a keyword like \ttx{north east}.  It is also possible to specify an
    explicit coordinate (potentially defined in terms of \ttc\glertnode,
    ...
    described above.  If \ttx{pos}=1 and \ttx{sep}=0pt (the defaults) then
    choosing \ttx{from=border\OR corner} creates a path of zero length, which
    cannot be used to orient the label.  If either is fractionally larger then
    the orientation angle is restored.  So a glert like \\
    \hspace*{1em} \ttx{label=\{[sep=0.1pt, from=corner, sloped]north west:\meta{label text}\}}
    \\
    will place a label at the corner oriented at $135^\circ$, while
    \\
    \hspace*{1em} \ttx{label=\{[pos=1.001, from=corner, sloped]north west:\meta{label text}\}}
    \\
    will orient the label to point to the node centre.
...
\item[\tt sloped\opt{=\meta{bool}}] Rotate the label to point towards \ttx{from}, by default
  ...
  \item{\ttx{inward}} Anchor to the east/west side father from the glert node,
    so that text runs inward to the node from the anchor.
  \item{\ttx{outward}} Anchor to the east/west side closer to the glert node so
    that text runs outward from the anchor away from the node.
  \end{description}
    
\item[\tt signal=\{\oarg{path options}\meta{angle}:\oarg{signal node
    options}\meta{signal text}\}] Use a \ttx{signal} node \ttx{label}.  All the
  label options apply.  In addition, the options
  \ttx{\string|>,>>,>\string|,<\string|,<<,\string|<} control the presence and
  orientation of signal pointers.  Right \ttx>s indicate pointers inwards to the
  glert node.
...
\begin{commandalt}[glert angle parser (tikz option)]{{tikz option: glert angle  parser}=\{\oarg{options}\meta{angle}:\oarg{target options}\meta{target}\}}{}
  ...
  effect depends on further parameters that can be set within
  \meta{options}: \ttx{pos=\meta{pos}}, \ttx{sep=\meta{sep}}.
...
\makeglertstyle{label}
  \defineglertpath{label}{active}{[glert angle parser={#1}] \glertlabelfrom -- \glertanglecoord 
