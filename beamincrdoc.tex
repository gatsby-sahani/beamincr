...
\def\pdfindxenv#1{\belowpdfbookmark{#1}{#1}}%
...
\def\extractenvironement#1#2@{\pdfindxenv{#1}\oldextractenvironment{#1}#2@}
...
\def\envwithcontents#1#2{%
  ...
  \extractflexenvironment#1@{#2}\par\topsep=0pt}
\def\endenvwithcontents{\endlist}
\def\extractflexenvironment#1#2@#3{%
  \pdfindxenv{#1}%
...
\end{command}
\vskip-3ex
\begin{commandalt}[\textbackslash incrlabel=//][incrlabelslash]{\incrlabel\sarg{incr}\opt{\meta{=}}\texttt{/}\meta{label}\texttt{/}}
...
\begin{commandalt}[increment specification]{{increment specification}: |label|\opt{|(offset)|}}
...
\begin{commandalt}[<resetincr@>]{{<resetincr@}\meta{increment}\cmdcontd{>}}
\end{commandalt}
\vskip-3ex
\begin{commandalt}[<!\meta{incr}>][<!incr>]{{<!}\meta{increment}\cmdcontd{>}}
...
environments.  There is no equivalent to the declaration form of |\onslide|.
Note, however, that |\fromslide| (\cref{sec:setting}) can act like an |\onslide|
declaration while also setting the current increment.
\end{commandalt}
...
If no \texttt/\meta{label}\texttt/ is found or if \meta{label} is empty, the
...
\begin{commandalt}[<\textasciitilde range>][tilde range]{{<\textasciitilde}\meta{increment
      range}\cmdcontd{\textpipe$\cdots$>}}

  This is equivalent to \texttt{<\meta{increment range}\textpipe
...
\begin{verbatim}
  ...
  \end{itemize}
\end{verbatim}
    The |\only| commands are used to advance the counter on slides where the
    |\item|s are visible.  In both cases the overlay specification to |\only| is a copy of the
    surrounding default specification.  For the first one, this is |<+->|.  For
    the second, it is |<+->| \emph{evaluated} within the |\item| call, giving
    |<3->|.  The final case expands the second |~| to the default |+-|, which
    becomes the increment specification for the item, and then sets the default
    within the item to its value, which is |4-|. 
\end{commandalt}
...
%
  ...
  processed with the action specification given by \sarg{pre-next specification}
  ...
  applied.  This avoids side effects from, e.g., |<+>| specifications in the
  default.  (The same does not apply to later empty \meta{next contents}: these
  are always set within action commands, triggering any side effects.)
...
\begin{envwithcontents}{{incrementaldocmd}
  ...
  |\next*| in place of |\next|.  Contents following |\next*| are set in the same
  way as in a plain |incremental| environment.
...
\begin{command}{{\CreateIncrementalAlignmentEnvironment}\marg{name}\oarg{Nopts}\marg{Nreqs}\opt{\texttt{[<}default
      specification\texttt{>]\oarg{base}}}}
...
equation numbering.
\begin{command}{\eqtag\sarg{overlay specification}\marg{tag}}
  Place \meta{tag} on slides that match \meta{overlay specification}.  
\end{command}
\begin{command}{\eqnum\sarg{overlay specification}}
  Place the current equation number (as |\theequation|) on slides that match
  \meta{overlay specification}, and then increment it.
\end{command}

The second form can be used as an action.
\begin{commandalt}[eqnum@]{{<eqnum@}\sarg{overlay specification}\cmdcontd{>}}
  Place the current equation number (as |\theequation|) on slides that match
  \meta{overlay specification}, and then increment it.
  \example
\begin{verbatim}
...
the environment will revert to unnumbered unless the appropriate \texttt{eqnum}
...
\section{Directing viewers' attention}\label{sec:point}

\subsection{Alerts}

\begin{command}{\alerts\sarg{overlay spec}\marg{argument contents}}
  Activate any |\alert*| commands in \meta{argument contents} contents on the
  overlays indicated by \meta{overlay spec}.
\end{command}

\begin{commandalt}[<alerts@spec>]{{<alerts@}\opt{spec}\textcolor{red}{\tt >}}
  This is the action equivalent of |\alerts|.
\end{commandalt}

\begin{command}{\alert*\marg{argument}}
  Executes |\alert|\marg{argument} if an enclosing |\alerts| command or action
  is active.  Otherwise \meta{argument} is displayed unalerted.
\end{command}
...
\begin{commandalt}[<point@spec>]{{<point@}\opt{spec}\textcolor{red}{\tt >}}
...
\begin{commandalt}[<pointers@spec>]{{<pointers@}\opt{spec}\textcolor{red}{\tt >}}
...
\begin{command}{{\point*}\oarg{options}\opt{\marg{contents}}}
  ...
  pointer is inserted as in the simple form, any options are ignored, and the
  contents is copied as is.
\end{command}
...
\begin{tabularx}{\textwidth}{>{\tt}l c X}
  ...
  \meta{label} & text & value assigned by |\incrlabel|\marg{label} or {\tt<=>}\\
  \meta{incrref}  & \meta{label}\ooffset  & = \tt value\{\meta{label}\}\poffset\\
                           & .\ooffset  & = \tt \meta{current increment}\poffset\\
                           & !\ooffset  & = \tt \meta{last increment reset}\poffset\\
  \meta{incr} & \tt /\meta{incrref}/ & = \texttt{value\{\meta{incrref}\}} (including \offset)\\
                      & \tt .\ooffset & = \tt\meta{current increment}\poffset\\
                      & \tt +\ooffset & = \tt++\meta{current increment}\poffset\\
  
  \meta{slide} & \meta{num}\|\meta{incr} & \\
  \meta{range} & \ometa{slide}\t-\ometa{slide} & (at least one \meta{slide} must be present)\\
                       & \tt /\meta{incrref}-/ & = {\tt0-0} if \meta{incrref}
  evaluates to 0 or is undefined and\newline |\allowundefinedincrlabels| is true\\
  
  <\meta{overlay spec}> & \multicolumn2l{} \\

  \spec <\meta{slide},\dots,\meta{range},\dots> & activate for \meta{slide}s or slides within \meta{range}s \\

  \spec |<~>| & copy default specification \\

  \spec <\meta{mode}:\meta{overlay spec}>
     & only apply \meta{overlay spec} in \beamer\ mode \meta{mode} \\
  \spec <\meta{overlay spec}\textpipe\meta{overlay spec}\textpipe \dots>
  ...
  \spec <resetincr@\meta{slide}> &  set \meta{current increment} to \meta{slide}\\

  \spec <!\meta{slide}\opt{-\meta{slide}}> 
    & = \tt <resetincr@\meta{slide}\textpipe\meta{slide}\opt{-\meta{slide}}>\\

  \spec <=\ooffset>\dots/\ooffset\meta{label}/ 
    & set \meta{label} to increment in effect after \meta{action spec} \opt{+\offset s}\\
  \spec <defaultspec@\meta{range}>
    & set default spec within argument to \meta{range} after evaluation in \meta{action spec}  \\
  \spec <|~|\meta{range}> &  = \tt <\meta{range}\textpipe defaultspec@\meta{range}> \\
  \spec <alert@\meta{range}> & |\alert| argument for slides in \meta{range}\\
  \spec <alerts@\meta{range}> & activate |\alert|\t* commands  in argument for slides in \meta{range}\\
  \spec <point@\meta{range}>  & |\point| and activate |\point|\t* commands in \meta{range}\\
  \spec <pointers@\meta{range}> & activate |\point|\t* commands in argument in \meta{range}\\
  \spec <eqnum@\meta{range}> & insert (and advance) |\theequation| tag for
  slides in \meta{range} \\

  \spec <\meta{mode}:\meta{action spec}> & only apply \meta{action spec} in \beamer\ mode \meta{mode} \\
  \spec <\meta{action spec}\textpipe\meta{action spec}\textpipe \dots> & apply
